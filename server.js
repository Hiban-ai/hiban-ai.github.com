  ...
  "rows": [
    {
      ...
      "deadline_days": "此列『完成期限』欄位的天數，數字字串。只有原文明確有完成期限（天數或期限日期）才填：天數直接用；期限日期換算成從今天起的剩餘天數(至少1)。⚠️ 絕不可拿『執行日期』來換算，該列只有執行日期而沒有完成期限時，此欄必須是空字串",
...
- ⚠️ deadline_days 只能來自「完成期限／期限／幾天內」這類欄位；該列只有執行日期、沒有完成期限時，deadline_days 一律空字串，絕不可由執行日期推算。
